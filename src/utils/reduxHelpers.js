...
/**
 * @desc Basic reducer that only response to set_state action
 * @param {object} originState
 * @param {String} reducerName
 */
export const reducer = (originState, reducerName) => (
  state = originState,
  action,
) => {
  const { type, target } = action;

  if (type === SET_STATE && target === reducerName)
 ...
 */
function setObjectValueByPath(object, key, value) {
  const recursiveSet = (object, chain, value) => {
    ...
    return {
      [key]: Object.assign(
        {},
        subObject,
        recursiveSet(subObject, chain, value),
      ),
    };
  };
...
export function isActionInNamespace(action, namespace) {
  return (
    action.namespace &&
    (action.namespace === namespace || action.namespace.includes(namespace))
  );
}
